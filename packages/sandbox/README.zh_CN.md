...
在 Apple Silicon (M3 Pro), macOS 15 上实测：

| 指标                 | @agentskillmania/sandbox |
| -------------------- | ------------------------ |
| 沙箱实例创建         | ~0.1ms                   |
| 首次命令执行         | ~60-130ms（WASM 加载 + 初始化，首次运行后缓存）|
| Shell 命令（热身后） | ~55ms                    |
| Python 命令（热身后）| ~55ms                    |
| 内存增长（50次执行） | ~0.3-3MB（取决于 GC）    |

*注意：每次执行都会生成新的 wasmtime 进程。~55ms 包含进程启动、WASM 实例化、命令执行和清理。这比 Docker ~100ms+ 的容器启动快得多，同时提供了相当的文件系统隔离。*
